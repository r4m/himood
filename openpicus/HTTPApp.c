...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
	...
	// STEP #2:
	// Handling of the cgi requests, in this case we have only "mood.cgi" but
	// it would be possible to have any other cgi request, depending on the webpage
	
	if(!memcmp(filename, "mood.cgi", 8))		// Is the requested file name "mood.cgi"?
	{
		// STEP #3:
		...
		// argument "led" from the request "mood.cgi?status=x" and we assign it to ptr.
		
		ptr = HTTPGetArg(curHTTP.data, (BYTE *)"status");
